...
// Source: https://github.com/bgavrilMS/msal-interactive-netcore/blob/master/active-directory-netcore-interactive-v2/OsBrowserWebUI/SingleMessageTcpListener.cs
...
namespace MigAz.Azure.ADAL
{
    ...
    internal class SingleMessageTcpListener : IDisposable
    {
        ...
        {
            string httpRequest = await GetTcpResponseAsync(tcpClient).ConfigureAwait(false);
            Uri uri = ExtractUriFromHttpRequest(httpRequest);

            // write an "OK, please close the browser message" 
            await WriteResponseAsync(responseProducer(uri), tcpClient.GetStream())
