...
namespace MigAz.Azure.Generator.ArmToArm
{
    public class ArmToArmGenerator : TemplateGenerator
    {
        ...
        public override void UpdateArtifacts(IExportArtifacts artifacts)
        {
            ...
            foreach (var VMv2 in _ARMArtifacts.VirtualMachines)
            {
                // string deploymentName;
                string virtualNetworkName;
                //                string loadBalancerName;

                _asmRetriever.GetARMVMDetails(VMv2.RGName, VMv2.VirtualMachine, out virtualNetworkName);
                ...
                var VMDetails = _asmRetriever.GetAzureARMResources("VirtualMachine", VMInfo, VMv2.RGName);
                var VMResults = JsonConvert.DeserializeObject<dynamic>(VMDetails);

                string location = VMResults.location;


                StatusProvider.UpdateStatus("BUSY: Exporting VM Config : " + VMv2.VirtualMachine);
                ...
                foreach (var nicint in VMResults.properties.networkProfile.networkInterfaces)
                {
                    ...
                    var NicDetails = _asmRetriever.GetAzureARMResources("NetworkInterface", NWInfo, null);
                    ...
                    if (NicResults.properties.ipConfigurations[0].properties.loadBalancerBackendAddressPools != null)
                    {
                        ...
                        BuildARMLoadBalancerObject(LBResults, PubIPName);
                    }
                }
                ...
                BuildARMAvailabilitySetObject(VMResults, virtualmachineinfo);

                // process network interface
                List<NetworkProfile_NetworkInterface> networkinterfaces = new List<NetworkProfile_NetworkInterface>();
                BuildARMNetworkInterfaceObject(VMResults, virtualmachineinfo, ref networkinterfaces);

                // process virtual machine
                BuildARMVirtualMachineObject(VMResults, virtualmachineinfo, networkinterfaces);
            }
            ...
            if (_settingsProvider.AllowTelemetry)
            {
                ...
                _telemetryProvider.PostTelemetryRecord(_TargetSubscription.AzureAdTenantId, this.Resources, _TargetSubscription.offercategories);
            }
        ...
        private void BuildARMVirtualNetworkObject(Arm.VirtualNetwork armVirtualNetwork, dynamic resource)
        {
            ...
            else
            {
                foreach (Arm.Subnet armSubnet in armVirtualNetwork.Subnets)
                {
                    ...
                    {
                        Core.ArmTemplate.NetworkSecurityGroup networksecuritygroup = BuildARMNetworkSecurityGroup(subnet.properties.networkSecurityGroup.id.Value);
                    ...
                    if (subnet.properties.routeTable != null)
                    {
                        RouteTable routetable = BuildARMRouteTable(subnet.properties.routeTable.id.Value);
        ...
        private NetworkSecurityGroup BuildARMNetworkSecurityGroup(string networksecuritygroupname)
        {
            ...
            var NSGDetail = _asmRetriever.GetAzureARMResources("NetworkSecurityGroup", nsginfo, null);
        ...
        private RouteTable BuildARMRouteTable(string routetablename)
        {
            ...
            var routeDetail = _asmRetriever.GetAzureARMResources("RouteTable", info, null);
        ...
        private void BuildARMVirtualMachineObject(dynamic resource, Hashtable virtualmachineinfo, List<NetworkProfile_NetworkInterface> networkinterfaces)
        {
            ...
            else
            {
                ...
                foreach (var SA in _ARMArtifacts.AllStorageAccounts)
                {
                ...
                var SAList = _asmRetriever.GetAzureARMResources("StorageAccountKeys", storageaccountinfo, RGName);
