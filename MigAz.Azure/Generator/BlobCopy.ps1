...
If ($StartType -eq "StartBlobCopy")
{
    ...
    foreach ($copyblobdetail in $copyblobdetails)
    {
        ...
        Start-AzureStorageBlobCopy -Context $source_context -ICloudBlob $snapshot.ICloudBlob -DestContext $destination_context -DestContainer $copyblobdetail.DestinationContainer -DestBlob $copyblobdetail.DestinationBlob -Verbose
